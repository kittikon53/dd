...
if(isset($_POST['username_account']) && isset($_POST['password_account'])){
    $username_account = htmlspecialchars(mysqli_real_escape_string($connect, $_POST['username_account'])); // รับชื่อผู้ใช้
    $password_account = htmlspecialchars(mysqli_real_escape_string($connect, $_POST['password_account']));
    $query_check_account = "SELECT * FROM account WHERE username_account = '$username_account'"; // แก้ไขให้เรียกดูจาก username_account
